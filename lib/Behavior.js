...
class Behavior {
    ...
    */
    ...
    running(event) {
        ...
        if (this.abortEvent.matches(event)) {
            return this.cancel(event);
        }

        // check if matches start?
        this.doRun(event);
        return false;
    }
    ...
    doRun(event) {
        if (this.state == RUNNING_INSIDE && this.affectedObject != null) {
            this.affectedObject.moveTo(
                this.originalX + (event.x - this.startX),
                this.originalY + (event.y - this.startY)
            );
            return true;
        }
        return false;
...
class ChoiceBehavior extends Behavior {
    ...
    */
    ...
    doStart(event) {
        if (this.state == RUNNING_INSIDE) {
            ...
            if (this.selectType == SELECT_MULTIPLE) {
                ...
                if (this.affectedObject != null) {
                    if (this.selectedObjects.has(this.affectedObject)) {
                        // if this object was previously selected, we need to deselect it
                        this.affectedObject.interimSelected = false;
                        this.affectedObject.selected = false;
                        this.selectedObjects.delete(this.affectedObject);
                    } else {
                        // not previously selected, select this object
                        this.affectedObject.interimSelected = true;
    ...
    doRun(event) {
        if (this.state == RUNNING_INSIDE) {
            if (!this.firstOnly) {
                // we can switch the selection while still running
                ...
                this.interimSelected.add(child);
                return true;
            }
        }
        return false;
